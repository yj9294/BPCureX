...
struct HeartRateReducer: Reducer {
...
struct HeartRateView: View {
    let store: StoreOf<HeartRateReducer>
    var body: some View {
        WithViewStore(store, observe: {$0}) { viewStore in
            VStack{
                AnalyticsFilterView(store: store.scope(state: \.filter, action: HeartRateReducer.Action.filter))
                ScrollView{
                    VStack{
            ...
            }.back(LocalizedStringKey(AnalyticsReducer.State.Item.heart.title)) {
                viewStore.send(.pop)
            }
        }
    }
            ...
            GeometryReader{ proxy in
                VStack(spacing: 15){
                    ForEach(source, id: \.self) { _ in
                        ...
                        }.stroke(style: .init(lineWidth: 1.0, dash: [3])).foregroundColor(Color("#E5EAED"))
                    }
                }
            }
        }
